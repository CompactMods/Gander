...
public class ParrotElement extends AnimatedSceneElement {
	...
	protected void renderLast(PonderLevel world, MultiBufferSource buffer, PoseStack ms, float fade, float pt) {
		...
		PoseTransformStack.of(ms).rotateYDegrees(AngleHelper.angleLerp(pt, entity.yRotO, entity.getYRot()));
	...
	public static abstract class ParrotPose {
		...
		Parrot create(PonderLevel world) {
			...
			Parrot.Variant variant = variants[world.random.nextInt(variants.length)];
